...
<body>
  ...
  <div class="grid-layout">
    <!-- CARD 1: Direct Excel Astro Report Upload -->
    <div class="card" style="border: 2px solid #8B008B;">
      <div class="card-title">
        <span>📊 1. UPLOAD DAILY ASTRO EXCEL REPORT (.XLSX)</span>
        <span style="font-size: 11px; color: #FFD700; background: rgba(139,0,139,0.3); padding: 4px 10px; border-radius: 6px;">⚡ Auto-Parses Nifty & Gold</span>
      </div>
      <div style="font-size: 12px; color: #CBD5E1; margin-bottom: 16px; line-height: 1.5;">
        Upload your formatted daily Astro Excel report (`.xlsx`) generated by the engine. The system automatically parses Nifty 50 & Gold XAUUSD reversal times, star ratings, day grade, volatility status, and active reversal window dates to update subscriber apps in real time.
      </div>
      
      <div class="form-group">
        <label class="form-label">📁 Select Daily Graded Excel File (.xlsx)</label>
        <div class="upload-zone" onclick="document.getElementById('file-astro-excel').click()">
          <div class="upload-label" id="lbl-astro-excel">📁 Click to Select Daily Graded Excel Report</div>
          <input type="file" id="file-astro-excel" accept=".xlsx, .xls" style="display:none;" onchange="handleFileSelect('file-astro-excel', 'lbl-astro-excel')">
        </div>
      </div>

      <button class="btn-push" style="background: linear-gradient(135deg, #8B008B, #6B21A8);" onclick="handleUploadAstroExcel()">⚡ BROADCAST EXCEL REPORT TO APP</button>
    </div>
