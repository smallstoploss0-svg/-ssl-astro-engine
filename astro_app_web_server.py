...
<body>
    ...
    <div id="client-dashboard" style="display: none; flex-direction: column; flex: 1;">
      ...
      <div class="content">
        ...
        <div id="admin-panel-container">
          ...
          <div style="font-weight: 900; font-size: 13px; margin-bottom: 6px; color: #8B008B;">📊 UPLOAD ASTRO EXCEL REPORT (.XLSX)</div>
          <div style="font-size: 11px; color: #64748B; margin-bottom: 10px;">Select daily report Excel file to update app reversal times & ratings instantly.</div>
          <div class="form-group">
            <input type="file" id="mob-astro-excel" accept=".xlsx, .xls" class="form-input">
          </div>
          <button type="button" style="background: #8B008B; color: white; border: none; width: 100%; padding: 13px; border-radius: 10px; font-weight: 900; font-size: 13px; cursor: pointer;" onclick="handleMobUploadAstroExcel()">⚡ BROADCAST ASTRO EXCEL</button>
  ...
  <script>
    ...
    async function handleMobUploadAstroExcel() {
      const file = document.getElementById('mob-astro-excel').files[0];
      if (!file) {
        alert('Please select an Excel (.xlsx) Astro Report file first!');
        return;
      }

      let formData = new FormData();
      formData.append('astro_excel', file);

      try {
        const res = await fetch('/api/upload_astro_excel', { method: 'POST', body: formData });
        const data = await res.json();
        alert('⚡ ' + data.message);
        loadPayloadData();
      } catch(e) {
        alert('⚠️ Error uploading Astro Excel report: ' + e);
      }
    }
...
<body>
  ...
  <!-- CARD 3: Direct Excel Astro Report Upload -->
  <div class="card" style="margin-bottom: 24px; border: 2px solid #8B008B;">
    <div class="card-title">
      <span>📊 3. UPLOAD DAILY ASTRO EXCEL REPORT (.XLSX)</span>
      <span style="font-size: 11px; color: #FFD700; background: rgba(139,0,139,0.3); padding: 4px 10px; border-radius: 6px;">⚡ Auto-Parses Nifty & Gold Slots</span>
    </div>
    <div style="font-size: 12px; color: #CBD5E1; margin-bottom: 14px; line-height: 1.5;">
      Upload your formatted daily Astro Excel report (`.xlsx`) generated by the engine. The system will automatically parse Nifty & Gold reversal times, star ratings, day grade, volatility status, and active reversal window dates to update subscriber apps in real time.
    </div>
    <div style="display: flex; gap: 14px; align-items: center;">
      <div style="flex: 1;" class="form-group">
        <label class="form-label">📁 Select Daily Graded Excel File (.xlsx)</label>
        <input type="file" id="file-astro-excel" accept=".xlsx, .xls" class="form-input">
      </div>
      <button class="btn-push" style="width: auto; padding: 14px 24px; margin-top: 10px; background: linear-gradient(135deg, #8B008B, #6B21A8);" onclick="handleUploadAstroExcel()">⚡ BROADCAST EXCEL REPORT TO APP</button>
    </div>
  </div>

  <!-- CARD 4: Live Subscriber Management Table -->
  <div class="card">
    <div class="card-title">
      <span>📋 4. LIVE SUBSCRIBERS LIST <span id="desk-total-badge" style="font-size:13px; color:#FFD700; background:rgba(255,215,0,0.15); padding:2px 8px; border-radius:6px; margin-left:8px;">(Total: 0 Subscribers)</span></span>
  ...
  <script>
    ...
    async function handleUploadAstroExcel() {
      const file = document.getElementById('file-astro-excel').files[0];
      if (!file) {
        alert('Please select an Excel (.xlsx) Astro Report file first!');
        return;
      }
...
class CustomHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def parse_astro_excel_report(self, excel_path):
        ...
        trend_str = "18-Sep & 19-Sep"

        for ws in wb.worksheets:
            rows = list(ws.iter_rows(values_only=True))
            for r in rows:
                if not r: continue
                row_vals = [str(c).strip() for c in r if c is not None]
                row_text = " ".join(row_vals)

                d_match = re.search(r'TODAY DATE\s*[\(\:]?\s*([\d\.\-\/]+)', row_text, re.IGNORECASE)
                if d_match:
                    date_str = d_match.group(1).replace('.', '-')

                t_match = re.search(r'TREND CHANGE DATES\s*[\(\:]?\s*([^\)]+)', row_text, re.IGNORECASE)
                if t_match:
                    trend_str = t_match.group(1).strip()

                if len(row_vals) >= 4:
                    sym = row_vals[0].upper()
                    time_val = row_vals[1] if len(row_vals) > 1 else ''
                    sess_val = row_vals[2] if len(row_vals) > 2 else ''
                    vol_val = row_vals[3] if len(row_vals) > 3 else 'NORMAL'
                    rating_val = row_vals[4] if len(row_vals) > 4 else '1.5 STAR ★½☆☆☆'

                    tm = re.search(r'(\d{1,2})[:\.](\d{2})\s*(AM|PM|am|pm)?', time_val)
                    if tm:
                        hr = int(tm[1])
                        mn = int(tm[2])
                        ampm = tm[3].upper() if tm[3] else ('PM' if 'PM' in time_val.upper() else 'AM')
                        ...
                        time_formatted = f"{hr12:02d}:{mn:02d} {period}"

                        is_master = "4.0" in rating_val or "MASTER" in rating_val.upper()
                        is_major = "2.5" in rating_val or "MAJOR" in rating_val.upper()
                        is_med = "2.0" in rating_val
                        ...
                        if 'NIFTY' in sym or 'NIFTY' in ws.title.upper():
                            n_slots_raw.append((slot_obj, vol_val))
                        elif 'GOLD' in sym or 'GOLD' in ws.title.upper() or 'XAUUSD' in sym:
                            g_slots_raw.append((slot_obj, vol_val))

        if n_slots_raw:
            n_slots = [item[0] for item in n_slots_raw]
            vol_n = n_slots_raw[0][1] if n_slots_raw else 'NORMAL'
            data['date'] = date_str
            data['nifty']['volatility_status'] = vol_n
            data['nifty']['active_reversal_window'] = trend_str
            data['nifty']['morning_slots'] = [s for s in n_slots if s['hour'] < 12 or (s['hour'] == 12 and s['minute'] == 0)]
            data['nifty']['afternoon_slots'] = [s for s in n_slots if s['hour'] > 12 or (s['hour'] == 12 and s['minute'] > 0)]

        if g_slots_raw:
            g_slots = [item[0] for item in g_slots_raw]
            vol_g = g_slots_raw[0][1] if g_slots_raw else 'NORMAL'
            data['gold']['volatility_status'] = vol_g
            data['gold']['active_reversal_window'] = trend_str
            data['gold']['asian_slots'] = [s for s in g_slots if s['hour'] < 12]
            data['gold']['london_slots'] = [s for s in g_slots if 12 <= s['hour'] < 18]
            data['gold']['ny_slots'] = [s for s in g_slots if s['hour'] >= 18]

        data['updated_at'] = datetime.now().isoformat()
        with open(payload_path, 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2)

        return True, f"Successfully parsed {len(n_slots_raw)} Nifty & {len(g_slots_raw)} Gold slots from Excel report!"
